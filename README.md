...
`daikome-jimusho.vercel.app`（司さんが 開く 事務所の 画面）の **入口**です。
中身は `vercel.json` の **rewrite 21本**だけで、
実際の 画面は **メーター側（daikou-app.vercel.app）** から 取ってきて 見せています。
...
```
16:08:01  Cloning github.com/exally-zeroact/Daikou-app (Branch: main)
...
- rewrite の 行き先は **本番のメーター** `https://daikou-app.vercel.app` だけ
  （テスト用は 別 repo `daikome-jimusho-host-test`）
- ★向き先を 取り違えると 事務所が テストを 見ます★ので、
  `tests/muki-saki.test.js` が 機械で 見張っています

## 元の 置き場

この `vercel.json` は `Daikou-app` の `office-host/vercel.json` から 持ってきました。
