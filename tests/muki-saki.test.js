...
//
//   ★これは 本番用の repo です★
//   ⇒ 行き先は ★本番のメーター（daikou-app.vercel.app）★だけ
//   ⇒★テストの URL が 1つでも 混ざったら 赤★
...
const KOCHIRA = 'https://daikou-app.vercel.app';
const MUKOU = 'https://daikou-app-test.vercel.app';
...
describe('★事務所の 入口の 向き先★', () => {
  ...
  it('★★② 行き先が 全部 本番のメーター★★', () => {
    ...
    expect(warui, '★本番以外を 向いている 行き先が あります★').toEqual([]);
  });

  it('★★③ テストの URL が 1つも 混ざっていない★★', () => {
    expect(raw.indexOf(MUKOU), '★テストの URL が 混ざっています★').toBe(-1);
    expect(raw.indexOf('-test.vercel.app'), '★テストの URL が 混ざっています★').toBe(-1);
  });
